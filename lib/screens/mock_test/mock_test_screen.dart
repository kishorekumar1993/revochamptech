// lib/screens/mock_test/mock_test_screen.dart
import 'dart:async';
import 'dart:convert';
...
class TestSEOConfig {
  ...
  String get canonicalUrl => 'https://revochamp.site/mock-test/$category/$testSlug';
  String get testSlug => testTitle.toLowerCase().replaceAll(' ', '-');
  String get ogImageUrl => 'https://revochamp.site/og-images/mock-test/$category.png';
  
  String get pageTitle => '$testTitle | $category Mock Test | RevoChamp Interview Prep';
  
  String get metaDescription => 
      'Take this $questionCount-question $category mock test. '
      'Time limit: $timeLimit minutes. Topics: ${topics.take(5).join(', ')}. '
      'Get instant AI feedback and detailed explanations. Prepare for FAANG interviews.';
  ...
  List<Map<String, String>> get faqs => [
    {
      "question": "What is this $category mock test?",
      "answer": "This is a comprehensive $category mock test with $questionCount questions covering ${topics.take(3).join(', ')}. It simulates real technical interviews with timed conditions and provides detailed explanations for each answer."
    },
    {
      "question": "How long does the test take?",
      "answer": "The test has a time limit of $timeLimit minutes. You can pause and resume, but we recommend completing it in one sitting to simulate real interview conditions."
    },
    {
      "question": "Will I get feedback on my answers?",
      "answer": "Yes! After each question, you'll receive immediate feedback with detailed explanations. At the end, you'll get a comprehensive report showing your strengths and areas for improvement."
    },
    {
      "question": "Is this test suitable for FAANG interview preparation?",
      "answer": "Absolutely! Our questions are curated by FAANG interviewers and cover the exact topics and difficulty levels you'll encounter in top tech company interviews."
    },
  ];
}
...
class TestSession {
  ...
  }) : questionMap = {for (var q in questions) q.id: q};

  int get totalPoints => questions.fold(0, (sum, q) => sum + q.points);
  int get beginnerCount => questions.where((q) => q.difficulty == 'beginner').length;
  int get intermediateCount => questions.where((q) => q.difficulty == 'intermediate').length;
  int get advancedCount => questions.where((q) => q.difficulty == 'advanced').length;
  List<String> get uniqueTopics => questions.map((q) => q.topic).toSet().toList();
...
class TestService {
  ...
  static final Map<String, TestSession> _cache = {};
  
  TestSession? getCachedTest(String category, String fileName) {
    final key = '$category/$fileName';
    return _cache[key];
  }
  ...
  Future<TestSession> loadTest({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final session = TestSession.fromJson(jsonData);
...
// ============================================================
// MAIN ENTRY POINT - OPTIMIZED FOR DEFERRED LOADING
// ============================================================
...
class _MockTestScreenState extends State<MockTestScreen> with WidgetsBindingObserver {
  ...
  late final Future<TestSession> _testFuture;
  
  TestSession? _cachedSession;
  TestSEOConfig? _seoConfig;
  ...
  void initState() {
    ...
    _cachedSession = _testService.getCachedTest(widget.category, widget.fileName);
    
    // Load test (from cache or network)
    _testFuture = _testService.loadTest(
  ...
  void _setupGlobalSEO() {
    if (!kIsWeb) return;
    if (_globalSEOSetup) return;
    
    _setDocumentLanguage();
    _addPreconnectUrls();
    _globalSEOSetup = true;
  }
  ...
  void _addPreconnectUrls() {
    ...
    final urls = ['https://json.revochamp.site', 'https://fonts.googleapis.com'];
  ...
  void _setupPageSEO(TestSession session) {
    if (!kIsWeb) return;
    
    _seoConfig = TestSEOConfig(
      category: _capitalize(widget.category),
      testTitle: session.title,
      questionCount: session.questions.length,
      timeLimit: session.timeLimitMinutes,
      topics: session.uniqueTopics,
    );
    
    _addH1Tag(session);
    _updateMetaTags();
    _setStructuredData(session);
    _setBreadcrumbSchema();
    _setFAQSchema();
  }

  void _addH1Tag(TestSession session) {
    if (!kIsWeb) return;
    ...
    final h1 = html.HeadingElement.h1()
      ..text = '${session.title} - ${_capitalize(widget.category)} Mock Interview Test'
      ..className = 'seo-h1'
  ...
  void _setStructuredData(TestSession session) {
    if (!kIsWeb) return;
    ...
    MetaService.setStructuredData({
      ...
      "educationalLevel": _getEducationalLevel(session),
      "timeRequired": "PT${session.timeLimitMinutes}M",
      "about": {
        "@type": "Thing",
        "name": widget.category,
        "description": "Technical interview preparation for ${widget.category}"
      },
      "hasPart": session.questions.take(10).map((q) => {
        "@type": "Question",
  ...
  void _setBreadcrumbSchema() {
    ...
    MetaService.setBreadcrumbData(
      title: _seoConfig?.testTitle ?? widget.fileName,
      slug: 'mock-test/${widget.category}/${widget.fileName}',
      parents: [
        {'name': 'Home', 'url': 'https://revochamp.site/tech/'},
        {'name': 'Mock Interview', 'url': 'https://revochamp.site/tech/mockinterview'},
        {'name': _capitalize(widget.category), 'url': 'https://revochamp.site/tech/mock-interview/${widget.category}'},
      ],
    );
  }
  ...
  String _getEducationalLevel(TestSession session) {
  ...
  void _trackPageView() {
    if (kReleaseMode) {
      ...
      debugPrint('📊 MockTestScreen [${widget.category}/${widget.fileName}] loaded in ${loadTime}ms');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_cachedSession != null) {
      return TestIntroScreen(session: _cachedSession!);
    }
    ...
    return FutureBuilder<TestSession>(
      ...
      builder: (context, snapshot) {
        ...
        // Setup SEO after data loads
        _setupPageSEO(snapshot.data!);
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        const Spacer(),
                        _skeletonBox(width: 80, height: 28, radius: 30),
                      ],
                    ),
                    const SizedBox(height: 20),
                    _skeletonBox(width: 70, height: 70, radius: 35),
                    const SizedBox(height: 16),
                    _skeletonBox(width: 200, height: 28, radius: 8),
                    const SizedBox(height: 8),
                    _skeletonBox(width: 300, height: 16, radius: 4),
                  ],
                ),
              ),
            ),
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 20),
                  _skeletonBox(height: 50, radius: 16),
                  const SizedBox(height: 20),
...
class TestIntroScreen extends StatelessWidget {
  ...
  Widget _buildHeader(BuildContext context, bool isMobile) {
    return Container(
      constraints: BoxConstraints(maxHeight: isMobile ? 220 : 320),
      decoration: BoxDecoration(
        gradient: const LinearGradient(
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              Row(
                children: [
                  _iconButton(Icons.arrow_back_ios, () => Navigator.pop(context)),
                  const Spacer(),
                  _categoryChip(),
                ],
              ),
              ...
              AnimatedContainer(
                ...
                child: const Icon(Icons.assignment_turned_in, size: 35, color: Colors.white),
              ),
              const SizedBox(height: 16),
              Text(
                session.title,
  ...
  Widget _buildStatsRow(bool isMobile) {
    return Row(
      children: [
        Expanded(child: _statCard(Icons.quiz, '${session.questions.length}', 'Questions')),
        const SizedBox(width: 10),
        Expanded(child: _statCard(Icons.timer, '${session.timeLimitMinutes}', 'Minutes')),
        const SizedBox(width: 10),
        Expanded(child: _statCard(Icons.stars, '${session.totalPoints}', 'Points')),
      ],
    );
  }
      ...
      children: [
        ...
        Row(
          children: [
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.only(
                    ...
                    bottomLeft: Radius.circular(
                      session.beginnerCount > 0 && session.intermediateCount == 0 && session.advancedCount == 0 ? 4 : 0
                    ),
                  ),
                ),
              ),
            ...
            Expanded(
              flex: session.intermediateCount,
              child: Container(height: 8, color: Colors.orange),
            ),
            Expanded(
              flex: session.advancedCount,
              child: Container(
                height: 8,
                decoration: BoxDecoration(
                  color: Colors.red,
                  borderRadius: BorderRadius.only(
                    topRight: Radius.circular(session.advancedCount > 0 ? 4 : 0),
                    bottomRight: Radius.circular(session.advancedCount > 0 ? 4 : 0),
                  ),
                ),
              ),
            ),
        ...
        Row(
          ...
          children: [
            _buildDifficultyLegend('Beginner', session.beginnerCount, Colors.green),
            _buildDifficultyLegend('Intermediate', session.intermediateCount, Colors.orange),
            _buildDifficultyLegend('Advanced', session.advancedCount, Colors.red),
          ],
        ),
      ],
    );
    ...
    return Column(
      ...
      children: [
        ...
        Wrap(
          spacing: 8,
          runSpacing: 8,
          children: session.uniqueTopics.map((topic) {
            return Chip(
              label: Text(topic),
  ...
  Widget _buildFeatures(bool isMobile) {
    return Column(
      ...
      children: [
        ...
        _feature("Detailed Explanations"),
      ],
    );
  }
  ...
  Widget _buildStartButton(BuildContext context, bool isMobile) {
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(builder: (_) => _MockTestQuizScreen(session: session)),
        );
      },
      child: Container(
